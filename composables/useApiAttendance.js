...
export const useApiAttendance = async () => {
    ...
    // call API login
    const { data: response, status } = await useFetch('http://localhost:3000/web/v1/attendance/get-attendance', {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${token}`
        },
        body: {
            start: "2024-05-01", // date or "-" (filter periode start)
            end: "2024-05-20", // date or "-" (filter periode end)
            id_division: "-",
            id_shifting: "-",
            id_account: [
