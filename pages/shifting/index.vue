<template>
    <div class="flex flex-col space-y-4 px-8 py-4 w-full overflow-y-auto">
        ...
        <div class="p-4 w-full">
            <h1 class="mb-2 font-bold text-3xl">Shifting</h1>
        ...
        <div class="flex flex-col bg-white shadow p-6 rounded-xl w-full">
            ...
            <div class="flex justify-between items-center">  
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-bold text-xl">Filter Shifting</span>
                </div>
            ...
            <div :class="isFilterWrap ? 'h-0' : 'h-[284px]'" class="transition-all duration-500 overflow-hidden">
                ...
                <div class="gap-6 grid grid-cols-12 p-6 w-full">
                    <InputSelect v-model="selectedDivision" :dataSelect="dataSelectDivision" class="flex flex-col col-span-6 w-full" />
                    <InputSelect v-model="selectedShift" :dataSelect="dataSelectShift" class="flex flex-col col-span-6 w-full" />
                    <InputSelect v-model="selectedShiftType" :dataSelect="dataSelectShiftType" class="flex flex-col col-span-6 w-full" />
                    <InputCheckbox v-model="selectedStatus" :dataSelect="dataSelectStatus" class="flex flex-col col-span-6 w-full" />
                </div>
        ...
        <div class="flex flex-col space-y-6 bg-white shadow p-6 rounded-xl w-full">
            <div class="flex justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-bold text-xl">Data Shifting</span>    
                </div>
                <div class="flex gap-4">
                    <ButtonBase label="Export" icon="i-flowbite-printer-outline" />
                    <ButtonBase label="Tambah Shifting" icon="i-flowbite-circle-plus-outline" />
                </div>
            </div>
                    ...
                    <tr v-for="(item, index) in dataTable" class="odd:bg-slate-100 even:bg-white border-b">
                        ...
                        <td class="px-2 font-medium text-center text-slate-800">
                            <div class="flex flex-row gap-2">
                                <NuxtLink :to="`/shifting/${index + 1}`" class="flex justify-center items-center bg-slate-50 hover:bg-slate-100 m-auto border rounded-full w-10 h-10">
...
<script setup>
    ...
    // data select: shift type
    const selectedShiftType = ref('-');
    const dataSelectShiftType = {
        info: {
            label: "Tipe Shift",
            placeholder: "Pilih Semua Tipe Shift"
        },
        options: [
            {
                value: 1,
